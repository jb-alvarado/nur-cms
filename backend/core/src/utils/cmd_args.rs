...
pub struct Args {
    ...
    #[clap(short, long, help = "Override the configured listen address")]
    pub listen: Option<String>,

    #[clap(long, help = "Disable two-factor authentication")]
...
pub async fn add_user(pool: &Pool<Postgres>) -> Result<(), NurError> {
    ...
    handles::insert_record::<_, AuthUser, i32>(pool, &Table::AuthUsers, &user).await?;
